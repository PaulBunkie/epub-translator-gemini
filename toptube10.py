...
# Константы
DAYS = 5
Q_TEMPLATE = "interview|интервью|беседа|обзор|разговор|репортаж|investigation|расследование|путешествие|travel|итоги"

# Словарь игровых ключевых слов для исключения
# Добавляйте сюда новые игровые ключевые слова по мере необходимости
# Система будет автоматически исключать видео, содержащие эти слова в заголовке или названии канала
GAMING_KEYWORDS = [
...
# Список ключевых слов для отсева нецелевого регионального контента (Индия, Пакистан и т.д.)
NON_TARGET_KEYWORDS = [
...
class TopTubeManager:
    ...
    def _should_save_video(self, video: Dict[str, Any], channels_dict: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            # Фильтруем игровой контент по ключевым словам (в заголовке и названии канала)
            title = video["snippet"]["title"].upper()
            channel_title = video["snippet"]["channelTitle"].upper()
            
            for keyword in GAMING_KEYWORDS:
                upper_keyword = keyword.upper()
                if upper_keyword in title:
                    print(f"[TopTube] Видео содержит игровое ключевое слово '{keyword}' в заголовке: {video['snippet']['title'][:50]}... — пропускаем")
                    return False
                if upper_keyword in channel_title:
                    print(f"[TopTube] Канал содержит игровое ключевое слово '{keyword}': {video['snippet']['channelTitle']} — пропускаем")
                    return False
            
            # Фильтруем нецелевой региональный контент (Индия и др.)
            for keyword in NON_TARGET_KEYWORDS:
                upper_keyword = keyword.upper()
                if upper_keyword in title:
                    print(f"[TopTube] Видео содержит нецелевое слово '{keyword}' в заголовке: {video['snippet']['title'][:50]}... — пропускаем")
                    return False
                if upper_keyword in channel_title:
                    print(f"[TopTube] Канал содержит нецелевое слово '{keyword}': {video['snippet']['channelTitle']} — пропускаем")
                    return False
...
- Игровые механики, прохождения (walkthrough), летсплеи, геймплей
- Стримы игр, киберспорт, турниры по играм
- Обзоры игр, новости игровой индустрии, игровое оборудование
- Любой контент, где в заголовке есть названия игр или термины: "босс", "уровень", "скины", "моды", "сервер", "рпг", "шутер"
...
Будь КРАЙНЕ ИЗБИРАТЕЛЕН. Игнорируй любой мусор, хайп и детский контент.

Меня интересуют ТОЛЬКО серьезные и познавательные видео на темы:
- Общество, политика, экономика, глобальные тренды
- Культура, искусство, история, философия
- Наука, высокие технологии, медицина, образование
- Качественные путешествия, урбанистика, стиль жизни
- Глубокие интервью с экспертами, аналитиками, учеными
- Документальные фильмы, серьезные расследования
- Глубокие аналитические обзоры

Меня КАТЕГОРИЧЕСКИ НЕ ИНТЕРЕСУЮТ (ИСКЛЮЧАЙ):
- ЛЮБОЙ ИГРОВОЙ КОНТЕНТ (игры, стримы, киберспорт, прохождения)
- ЛЮБОЙ ИНДИЙСКИЙ КОНТЕНТ (даже с миллионами просмотров - Болливуд, индийские шоу, влоги, музыка)
- Котики, пранки, челленджи, тикток-хайп
- Бытовые обзоры дешевых товаров и услуг
- Телесериалы, мыльные оперы, развлекательные ток-шоу
- Музыкальные клипы, поп-культура (если это не серьезный разбор)
- Детский контент, мультфильмы, мемы
- Видео с кричащими заголовками "ШОК", "СМОТРЕТЬ ВСЕМ" и т.п.

В ответе укажи ТОЛЬКО номера выбранных видео через запятую (например: 1, 3, 7, 12, 15). 
Если не можешь выбрать ни одного действительно достойного варианта, ответь "нет".
