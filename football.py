...
class FootballManager:
        ...
        try:
            ...
            cursor.execute("""
                SELECT id, fixture_id, home_team, away_team, match_date, match_time,
                       ...
                       live_odds, sofascore_event_id, status
                FROM matches
            ...
            for row in rows:
                ...
                if sofascore_eid:
                    ...
                    if ss_data and 'event' in ss_data:
                        ...
                        if status_type == 'finished' and h_val is not None and a_val is not None:
                            ...
                            # Отправляем push об окончании матча
                            if FIREBASE_PUSH_AVAILABLE:
                                try:
                                    k0_done = str(row['initial_odds']) if row['initial_odds'] else ""
                                    k1_done = str(row['last_odds']) if row['last_odds'] else ""
                                    k60_done = str(row['live_odds']) if row['live_odds'] else ""
                                    ...
                                    print(f"[FAVOURITE_TRACKING] 📲 Push MATCH_FINISHED sent | fixture={row['fixture_id']} | score={h_val}-{a_val} | k0={k0_done} | k1={k1_done} | k60={k60_done}")
                                except Exception as _fb_done_err:
                                    print(f"[FAVOURITE_TRACKING] ❌ Push MATCH_FINISHED FAILED | fixture={row['fixture_id']} | error={_fb_done_err}")
                ...
                # ===== ПОЛУЧАЕМ live минуту и сохраняем в БД =====
                live_minute = None
                fav_team_id = row['fav_team_id'] if 'fav_team_id' in row.keys() else None
                ...
                if sofascore_eid:
                    ...
                    if live_minute is not None:
                        ...
                        # Fallback: вычисляем по времени начала матча
                        try:
                            match_start = datetime.strptime(f"{row['match_date']} {row['match_time']}", "%Y-%m-%d %H:%M").replace(tzinfo=timezone.utc)
                            live_minute = int((datetime.now(timezone.utc) - match_start).total_seconds() / 60)
                            live_minute = max(0, min(90, live_minute))
                            print(f"[FAVOURITE_TRACKING] ⏱️ Fallback minute calculated | fixture={row['fixture_id']} | minute={live_minute}")
                        except Exception:
                            pass
                # Сохраняем минуту в БД
                if live_minute is not None:
                    try:
                        cursor.execute("UPDATE matches SET live_minute = ? WHERE id = ?", (live_minute, row['id']))
                        conn.commit()
                    except Exception:
                        pass
                ...
                if FIREBASE_PUSH_AVAILABLE:
                    try:
                        ...
                        k0 = str(row['initial_odds']) if row['initial_odds'] else ""
                        k1 = str(row['last_odds']) if row['last_odds'] else ""
                        k60 = str(row['live_odds']) if row['live_odds'] else ""
                        firebase_notifier.send_match_update(
                            match_id=str(sofascore_eid),
                            score_home=str(h_val),
                            score_away=str(a_val),
                        ...
                        print(f"[FAVOURITE_TRACKING] 📲 Push sent | fixture={row['fixture_id']} | sofascore_eid={sofascore_eid} | event_type={event_type} | score={h_val}-{a_val} | minute={live_minute} | k0={k0} | k1={k1} | k60={k60}{' ⚽ GOAL!' if is_goal else ''}")
    ...
    def _check_and_notify_favorite_losing(
        ...
        """
        Проверяет, проигрывает ли фаворит (счёт против него), и отправляет уведомление один раз.
        Ловит момент, когда фаворит начинает уступать.
...
⭐ <b>Фаворит:</b> {fav_team_name}
    ...
    def _check_and_notify_favorite_draw_at_minute(
        self,
        fixture_id: str,
        home_team: str,
        away_team: str,
        ...
        away_score: int,
        minute_elapsed: int,
        initial_odds: Optional[float],
        last_odds: Optional[float],
    ) -> None:
        """
        Отправляет уведомление один раз, когда фаворит играет вничью на заданной минуте (например 30-й).
        ...
        try:
            ...
            subscribers = get_football_subscribers()
            if not subscribers:
                print(f"[Football] Нет подписчиков для уведомления «фаворит вничью на {minute_elapsed}-й минуте» {fixture_id}")
                return
            odds_line = ""
            if initial_odds is not None and last_odds is not None:
                odds_line = f"\n📈 Начальный кэф: {initial_odds:.2f} → текущий: {last_odds:.2f}"
            elif last_odds is not None:
                odds_line = f"\n📈 Кэф на победу фаворита: {last_odds:.2f}"
            message = f"""⏱️ <b>Фаворит вничью на {minute_elapsed}-й минуте</b>

🏟️ <b>Матч:</b> {home_team} vs {away_team}
📊 <b>Счёт:</b> {home_score} - {away_score}
⭐ <b>Фаворит:</b> {fav_team_name}{odds_line}""".strip()
            success_count = 0
            for recipient_id in subscribers:
                if telegram_notifier.send_message_to_user(recipient_id, message):
                    success_count += 1
    ...
    def _search_sofascore_event_by_team(self, home_team: str, away_team: str, match_date: Optional[str] = None, match_time: Optional[str] = None) -> Optional[Dict]:
        """
        Поиск события в SofaScore по названию команд через /search/events.
        Возвращает dict с event_id, homeTeamId, awayTeamId или None.
        Дополнительно проверяет startTimestamp, если переданы match_date/match_time.
        """
        import random
        from datetime import timezone

        def _match_teams(h_name_raw, a_name_raw, target_home, target_away):
            """Проверяет совпадение обеих команд (только прямой порядок)."""
        ...
        def _parse_match_dt():
            if not match_date or not match_time:
                return None
            try:
                return datetime.strptime(f"{match_date} {match_time}", "%Y-%m-%d %H:%M").replace(tzinfo=timezone.utc)
            except Exception:
                return None

        expected_dt = _parse_match_dt()
        ...
        def _search(query_team):
            ...
            for item in results:
                ...
                if _match_teams(h_name, a_name, home_team, away_team):
                    ...
                    if ts and expected_dt:
                        try:
                            evt_dt = datetime.fromtimestamp(ts, tz=timezone.utc)
                            delta_hours = abs((evt_dt - expected_dt).total_seconds()) / 3600.0
                            if delta_hours > 8:
                                continue
                        except Exception:
                            pass
    ...
    def update_sofascore_ids(self) -> Dict[str, int]:
        ...
        try:
            ...
            for date_str in dates:
                ...
                for row in rows:
                    d = dict(row)
                    sr = self._search_sofascore_event_by_team(d['home_team'], d['away_team'], d.get('match_date'), d.get('match_time'))
                    if sr:
                        eid = sr['event_id']
                    ...
                    try:
                        ...
                        if alt_result:
                            ...
                            if bet_alt_code and bet_alt_odds and bet_alt_odds > 1.75 and bet_alt_confirm == 1:
                                ...
                                """, (match['id'],))
                                match_for_notification = cursor_notify.fetchone()
                                conn_notify.close()
        ...
        try:
            ...
            if bet_ai_reason:
                ...
                try:
                    ...
                    print(f"[Football Notify] Проверка условий для уведомления (фаворит, bet_ai): bet_ai={bet_ai}, bet_ai_odds={bet_ai_odds if bet_ai_odds else 'None'}")
                    if (db_row and db_row['bet_ai'] and db_row['bet_ai_odds'] and db_row['bet_ai_odds'] > 1.50):
                        ...
                        reason = []
                        if not db_row: reason.append("нет данных в БД")
                        elif not db_row['bet_ai']: reason.append("нет bet_ai")
                        elif not db_row['bet_ai_odds']: reason.append("нет bet_ai_odds")
                        elif db_row['bet_ai_odds'] <= 1.50: reason.append(f"bet_ai_odds={db_row['bet_ai_odds']} <= 1.50")
                        print(f"[Football Notify] ❌ Уведомление НЕ отправлено для матча с фаворитом {match['fixture_id']}: {', '.join(reason) if reason else 'условия не выполнены'}")
...
# <<< ВРЕМЕННО ДЛЯ ТЕСТА ВИДЖЕТА: убран fav != 'NONE', фаворит вычисляется по коэффициентам >>>
def get_favorites_today_tomorrow() -> List[Dict[str, Any]]:
    """
    ВРЕМЕННО: Получает ВСЕ матчи на ближайшие 10 дней.
    Фаворит вычисляется по меньшему коэффициенту из live_odds_1/live_odds_2 если не задан в БД.
    ...
    try:
        ...
        for row in all_rows:
            ...
            # ВРЕМЕННО: фаворит вычисляется по коэффициентам если не задан
...
'status': row['status'] if row['status'] else 'scheduled',
        ...
        if registry_conn:
            try:
                ...
                pass
# <<< КОНЕЦ ВРЕМЕННОГО ТЕСТА ВИДЖЕТА >>>
